...
class Timeslot extends \Opencart\System\Engine\Controller {
	public function index() {
		...
		if(!empty($tmd_timeslot_slot_from)){
	...
	public function productcontroller(string &$route, array &$args, mixed &$output): void {
		...
		if(VERSION>='4.0.2.0'){
			...
			$find='{% if subscription_plans %}';
			$replace='{{ timeslotdata }}'.'{% if subscription_plans %}';
  ...
  public function catalogaccount(string &$route, array &$args, mixed &$output): void {
			...
			foreach ($products as $product) {
				...
				$subscription_info = $this->model_account_subscription->getProductByOrderProductId($args['order_id'], $product['order_product_id']);
