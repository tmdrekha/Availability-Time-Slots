<?php
namespace Opencart\Catalog\Model\Extension\Tmdavailabilitytimeslots\Tmd;
...
class Timeslot extends \Opencart\System\Engine\Model {
	...
	public function getCartdata($product_id){
		$cart_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "cart` WHERE `api_id` = '0' AND `product_id` = '".(int)$product_id."' AND `session_id` = '" . $this->db->escape($this->session->getId()) . "'ORDER BY cart_id DESC");
